        ...
        if (!data.loggedIn) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    document.getElementById('nombres').value = data.firstName;
                    document.getElementById('apellidos').value = data.lastName;
                    document.getElementById('usuario').value = data.username;
                    document.getElementById('email').value = data.email;
        
                    // Convertir la fecha a "yyyy-MM-dd" para el campo de fecha
                    const birthdate = new Date(data.birthdate);
                    const formattedBirthdate = birthdate.toISOString().split('T')[0];
                    document.getElementById('cumpleaños').value = formattedBirthdate;
                } else {
